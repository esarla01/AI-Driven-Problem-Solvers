...
class Node:
    ...
    # heuristic function: (forward cost) returns the number of pairs in the pancake stack that are not consequtive numbers.
    def heuristic_function(self):
        num_gap = 0
        arr = copy.deepcopy(self.array)
        plate = len(arr) + 1
      
        if (not abs(plate - self.array[0]) == 1): #let the plate count as a value that is 1 bigger than the biggest value in the array
            num_gap +=1  # count the plate into the gap calculation to minimize the time to find the goal states
    ...
    # total cost function: = cost function + heuristic function
    def total_cost_func(self):
        #print(self.backward_cost)
        #print(self.heuristic_function())
        return self.backward_cost + self.heuristic_function()
    ...
    def __lt__(self, other):
        bol_equal = not (self.total_cost_func() == other.total_cost_func())
        if bol_equal:
                return self.total_cost_func() < other.total_cost_func()
...
print("You will enter up to 7 numbers from 1-7 in a random order! The numbers should have a comma and then a space in between each other!" + '\n')
...
# array = [1, 2, 5, 4, 3, 6, 9, 10, 7, 8]

# array = [1, 2, 3, 4, 5, 6, 7]

# test the initialization of the class
# test_nodes = Node(array, 0, 3)

#print(repr(test_nodes))

#print(test_nodes.backward_cost)

#print(test_nodes.heuristic_function())

# test total cost function
#print(test_nodes.total_cost_func())

# test children
# children = test_nodes.children_nodes()
# print(children)


# astar = A_Star(array)
# leaf1 = astar.frontier.pop()
# astar.expand_frontier(leaf1)


# for child in astar.frontier.heap:
#     print(child)
    # print(child.backward_cost)
    # print(child.heuristic_function())
    # print(child.total_cost_func())
    # print(child.rank)
